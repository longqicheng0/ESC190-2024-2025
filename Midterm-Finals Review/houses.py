...
for i in range (1,N):
    ...
    cost[0][i] = houses[1][i] + min(cost[0][i - 1], cost[2][i - 1]) 
    cost[0][i] = houses[2][i] + min(cost[0][i - 1], cost[1][i - 1])
...
#solution dictionary
#sol[col][i]: ie what to paint house i - 1 if house i is painted col
